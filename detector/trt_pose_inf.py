...
# pycuda.autoinit 作用，在导入 pycuda.driver 之前自动初始化 CUDA 上下文，确保后续的 CUDA 调用能够正常工作
# 多线程程序避免免在主线程中初始化 CUDA 上下文，应推迟到需要使用 GPU 的线程中进行初始化
# 最好手动管理 CUDA 上下文，确保每个线程都正确初始化和清理上下文，避免潜在的资源冲突和错误
