...
- npy: 4x4 ground truth object pose in robot base frame (bMo)
"""
import os
import cv2
import numpy as np
...
# Object rotation: 180 deg around X (from infer.py)
...
def main():
    ...
    npy_files = sorted([f for f in os.listdir(IMG_DIR) if f.endswith('.npy') and f != 'temp'])
    png_files = {f.replace('.png', ''): f for f in os.listdir(IMG_DIR) if f.endswith('.png')}

    # Header
    print(f"{'File':<50} {'method':<6} {'t_x':>10} {'t_y':>10} {'t_z':>10} "
          f"{'r_x':>10} {'r_y':>10} {'r_z':>10} {'reproj':>10}")
    print("-" * 120)

    for npy_file in npy_files:
        ts = npy_file.replace('.npy', '')
        if ts not in png_files:
            continue

        img_path = os.path.join(IMG_DIR, png_files[ts])
        bMo_gt = np.load(os.path.join(IMG_DIR, npy_file))

        img = cv2.imread(img_path)
        if img is None:
            print(f"{npy_file}: failed to load image")
            continue
        ...
        if len(results) == 0 or len(results[0].boxes) == 0:
            print(f"{npy_file}: no YOLO detection")
            continue

        roi_box = results[0].boxes.xyxy[0].cpu().numpy()
        ...
        if final_pts is None:
            print(f"{npy_file}: matching failed, score={score}")
            continue
        ...
        # Camera pose from PnP
        cMo_pnp = np.eye(4, dtype=np.float64)
        cMo_pnp[:3, :3] = Rotation.from_rotvec(result['rvec_pnp']).as_matrix()
        cMo_pnp[:3, 3] = result['tvec_pnp'].flatten()
        ...
        cMo_pnp_ = cMo_pnp @ oMo
        cMo_ba_ = cMo_ba @ oMo

        bMo_gt_euler = Rotation.from_matrix(bMo_gt[:3, :3]).as_euler('xyz', True)

        cMo_pnp_euler = Rotation.from_matrix(cMo_pnp[:3, :3]).as_euler('xyz', True)
        cMo_ba_euler = Rotation.from_matrix(cMo_ba[:3, :3]).as_euler('xyz', True)

        elapsed = (time.perf_counter_ns() - t0) / 1e6

        print(f"\n--- {npy_file} ({elapsed:.1f}ms) ---")
        print(f"{'GT(bMo)':<50} {'PnP':<6} {bMo_gt[0,3]:>10.2f} {bMo_gt[1,3]:>10.2f} {bMo_gt[2,3]:>10.2f} "
              f"{bMo_gt_euler[0]:>10.2f} {bMo_gt_euler[1]:>10.2f} {bMo_gt_euler[2]:>10.2f} {'-':>10}")
        print(f"{'[cMo]':<50} {'PnP':<6} {cMo_pnp[0,3]:>10.2f} {cMo_pnp[1,3]:>10.2f} {cMo_pnp[2,3]:>10.2f} "
              f"{cMo_pnp_euler[0]:>10.2f} {cMo_pnp_euler[1]:>10.2f} {cMo_pnp_euler[2]:>10.2f} {result['reproj_err_pnp']:>10.4f}")
        print(f"{'[cMo]':<50} {'BA':<6} {cMo_ba[0,3]:>10.2f} {cMo_ba[1,3]:>10.2f} {cMo_ba[2,3]:>10.2f} "
              f"{cMo_ba_euler[0]:>10.2f} {cMo_ba_euler[1]:>10.2f} {cMo_ba_euler[2]:>10.2f} {result['reproj_err_ba']:>10.4f}")
