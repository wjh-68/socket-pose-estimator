...
FIXED_ERROR_THRESHOLD = 0.6    # used when USE_ADAPTIVE_THRESHOLD = False
...
if __name__ == '__main__':
    ...
    for record in records:
        ...
        if final_pts is not None and centers.shape[0] >= 7:  # need at least 4 points for solvePnP_IPPE
            ...
            # Print per-point reprojection errors of pnp
            ...
            for i, (err, pt_name) in enumerate(zip(per_point_errors_pnp, obj_pt_names[:len(per_point_errors_pnp)])):
                marker = '[INLIER]' if (inlier_mask is not None and inlier_mask[i]) else '[OUTLIER]'
                print(f"  Point {i} ({pt_name}): {err:7.3f} px {marker}")
            
            # Print per-point reprojection errors of cMo based last optimized pose (if available)
            if last_bMo is not None and optimizer.is_initialized():
                ...
                # if the diff between pose based pnp and pose based on last optimized is large, print a warning
            ...
            # Add frame to optimizer and optimize pose, but only if reprojection error is reasonable (to avoid bad local minima)
            ...
            # transform[:3,:3] = Rotation.from_rotvec(np.array([1,0,0])*np.pi).as_matrix()
            # transform[3,:3] = np.array([0,10,10])

            # cMo_trans = cMo @ transform
            # cv2.drawFrameAxes(img, K, dist, cMo_trans[:3, :3], cMo_trans[:3, 3:], 15, 2)
